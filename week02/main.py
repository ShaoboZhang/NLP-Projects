...
class BiDAF:
    ...
    def build_model(self):
        ...
        ############################### 添加的代码片段 ###############################
        # 获取glove词向量与单词索引
        word2idx, embed_matrix = preprocess.load_glove(vocab_size=self.max_features)
        # 通过glove实现word embedding layer
        word_embed_layer = tf.keras.layers.Embedding(len(word2idx), self.emb_size, weights=[embed_matrix], trainable=False)
        # char-cnn embedding
        charcnn_embed_layer = tf.keras.Sequential([
            tf.keras.layers.Embedding(self.max_features, self.emb_size, embeddings_initializer='uniform'),
            tf.keras.layers.Conv1D(self.max_features, kernel_size=5, activation='tanh')
        ])
        # 拼接word embedding与char cnn embedding
        embed_layer = tf.keras.layers.Concatenate(axis=-1)([word_embed_layer, charcnn_embed_layer])
        qemb = embed_layer(qinn)
        cemb = embed_layer(cinn)
        ############################### 添加的代码片段 ###############################
