...
class BiDAF(nn.Module):
    def __init__(self, args):
        ...
        self.char_conv_pooling = nn.Sequential(
        ...
        # Word Embedding, replace embedding by bert
        self.word_emb = BertModel.from_pretrained(args.bert_model_path)
    ...
    def forward(self, input_ids, token_type_ids, attn_mask, c_chars, q_chars):
        def char_embed_layer(x):
            ...
            char_conv = self.char_conv_pooling(char_embed)
        ...
        def highway_network(x1, x2):
            ...
            :return: (batch_sz, seq_len, hidden_size)
            """
            # (batch_sz, seq_len, 2*embed_sz)
        ...
        # word embedding
        word_embed = self.word_emb(input_ids, token_type_ids, attn_mask)[0]

        c_word_embed = word_embed[:, 1:self.args.max_seq_clen + 1, :]
        q_word_embed = word_embed[:, self.args.max_seq_clen + 2:-1, :]
